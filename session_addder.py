...
class SessionAdder():
    ...
    def _add_sid_to_group(cls:"SessionAdder", group:DataFrame) -> DataFrame:
        ...
        group["session_id"] = np.where(group["timestamp"] - group["timestamp"].shift(1) > pd.Timedelta(minutes=3), 1, 0).cumsum() + cls.used_sid
    ...
    def generate_df(n_customers:int ,n_products:int, n_rows:int, start:str, end:str, file_path:str, save_to_file: bool = True) -> None:
        ...
        random_data = {
            ...
            "timestamp": pd.to_datetime(np.random.randint(pd.to_datetime(start).value // 10**9, pd.to_datetime(end).value // 10**9, n_rows), unit="s")
        }
...
if __name__ == "__main__":
    ...
    # Генерируем датафрейм, дополнительно сохраняя его в файл
    df = SessionAdder.generate_df(n_customers ,n_products, n_rows, start, end, input_file_path, save_to_file = True)
